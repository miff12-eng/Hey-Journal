// Replit Auth OAuth integration - based on blueprint:javascript_log_in_with_replit
    ...
    req.logout(() => {
      res.redirect(
        client.buildEndSessionUrl(config, {
          ...
          post_logout_redirect_uri: `${req.protocol}://${req.hostname}/public`,
        }).href
      );
    });
  });
...
export const isAuthenticated: RequestHandler = async (req, res, next) => {
  ...
  try {
    ...
    // Persist the updated tokens to the session
    req.login(user, (err) => {
      if (err) {
        return res.status(401).json({ message: "Unauthorized" });
      }
